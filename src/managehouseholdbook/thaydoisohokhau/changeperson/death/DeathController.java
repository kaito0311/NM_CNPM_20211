...
public class DeathController implements Initializable {
    ...
    public boolean checkIsHead(){
        personIDdeath = takePersonID(this.nameDeathPerson, Integer.toString(this.bookIDDeathPerson), this.birthDateDeathPerson.toString());
            if(personIDdeath == -1){
                return false;
            }
            personDeath.setText(this.nameDeathPerson);
            // Kiểm tra thông tin chủ hộ nếu là chủ hộ yêu cầu thay đổi thông tin chủ hộ rồi mới khai tử :) 
