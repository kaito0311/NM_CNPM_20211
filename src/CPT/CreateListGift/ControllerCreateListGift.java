 ...
 * @author Cool IT Help
 */
public class ControllerCreateListGift implements Initializable {
...
//    }
	...
	public void changeToGiftChild(ActionEvent event) {
		try {
			AnchorPane root = (AnchorPane)FXMLLoader.load(getClass().getResource("/CPT/Gift/UI_Gift.fxml"));
			stage = (Stage)((Node)event.getSource()).getScene().getWindow();
			scene = new Scene(root);
			stage.setScene(scene);
			stage.setX(220);
    ...
    try {
		...
		while(rs.next()) {
			...
			Person p = new Person(rs.getString("FullName"), rs.getString("BirthDate"), strGender, rs.getString("Age"), rs.getString("BookID"));
			listPerson.add(p);
		}
